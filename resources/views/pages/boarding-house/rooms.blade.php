...
<form action="{{ route('booking', $boardingHouse->slug) }}" class="relative flex flex-col gap-4 mt-5">
    ...
    <div id="RoomsContainer" class="flex flex-col gap-4 px-5">
        ...
        <label class="relative group">
            <input type="radio" name="room_id" value="{{ $room->id }}" class="absolute top-1/2 left-1/2 -z-10 opacity-0" required>
            <div
                ...
                <div class="flex flex-col gap-3 w-full">
                    ...
                    <hr class="border-[#F1F2F6]">
                    <p class="font-semibold text-lg text-ngekos-orange">{{ number_format($room->price_per_month, 0, ',','.') }}<span
