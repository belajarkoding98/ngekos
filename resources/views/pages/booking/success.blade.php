...
<div class="relative flex flex-col gap-[30px] my-[60px] px-5">
                ...
                <div class="flex flex-col gap-3 w-full">
                    ...
                    <div class="flex items-center gap-[6px]">
                        ...
                        <p class="text-sm text-ngekos-grey">
                            {{ \Carbon\Carbon::parse($transaction->room->start_date)->isoFormat('D MMMM YYYY') }}
                            {{ \Carbon\Carbon::parse($transaction->room->start_date)->addMonths($transaction->duration)->isoFormat('D MMMM YYYY') }}
                        </p>
                    </div>
                </div>
            </div>
    ...
    <div class="flex flex-col gap-[14px]">
        ...
        <form action="{{ route('check-booking') }}" method="POST">
            ...
            <button type="submit" class="w-full rounded-full p-[14px_20px] text-center font-bold text-white bg-ngekos-black">View My Booking</button>
        </form>
    </div>
</div>
@endsection